...
1. Build the backend jar (via IntelliJ Maven or the bundled Maven):
   `mvn -f FengYu-Api/pom.xml install -DskipTests && mvn -f plugin-markdown/pom.xml install -DskipTests && mvn -f FengYu/pom.xml package -DskipTests`
2. Copy the jar so the sidecar can find it:
   `mkdir -p src-tauri/binaries && cp ../FengYu/target/FengYu-4.0.0-SNAPSHOT.jar src-tauri/binaries/FengYu.jar`
   (If you skip this, `main.rs` falls back to `../../FengYu/target/FengYu-4.0.0-SNAPSHOT.jar`.)
...
- On launch: spawn `java ... --port=24056 --token=<generated>` → parse `FENGYU_PORT` → poll
