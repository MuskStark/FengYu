...
打包好的后端 jar 位于 `FengYu/target/FengYu-4.0.0-alpha.3.jar`。
...
npm run dev       # 默认：连接 IDE 启动的后端 http://127.0.0.1:24056
                  ...
                  #   （先用 ./mvnw -pl FengYu -am package -DskipTests -Drevision=4.0.0-alpha.3 构建）
...
发布标签（`v4.0.0-alpha.3`，以及后续的 stable/beta/rc）会触发一条 GitHub Actions 流水线，发布**未签名**的 Electron 安装包（Windows/macOS/Linux）和一个**可移植的 Web 分发包**。Web 压缩包直接从文件夹运行同一套后端 + 内嵌的 Vue SPA：
