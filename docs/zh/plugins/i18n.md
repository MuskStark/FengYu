...
Vue 插件应优先使用 `mountFengYuApp`；它会先建立环境事件订阅，再等待 ready 握手。如果需要
自行编写启动逻辑，必须保持这个顺序：
...
宿主可能在 iframe 刚加载时就发送第一条环境事件。如果直到 `await client.ready()` 之后才注册
监听器，就会产生竞态：初始事件可能丢失，插件会一直停留在兜底的深色/英文界面，即使宿主已经
是浅色/中文。环境事件可能只包含部分字段，因此应先与最近一次状态合并，再更新 Vuetify 和插件
自己的消息表。
