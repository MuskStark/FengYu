---
title: 官方插件——Markdown
description: fan.summer.markdown（v4.0.0）讲解——一个 text 类别插件，无权限、无 AI 工具，仅暴露一个由 MarkdownWorkerMain 支撑的 render 方法，并配有一个带实时预览的 Vuetify 分栏编辑器 UI。
lang: zh-CN
---
...
- 使用 iframe 内部的 `@infinia/plugin-ui` Vuetify 实例（MD3）构建，并通过 SDK 环境桥梁跟随宿主的主题与语言变化。
