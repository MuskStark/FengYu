...
[GitHub 发布](https://github.com/MuskStark/FengYu/releases/tag/v4.0.0-alpha.6)
:::
...
### ✨ 新增
- **AI 聊天现已支持 Codex 风格的动作审批档位。** 输入栏提供「需审批」「代我审批」「完全访问」
  三档；命令执行与插件声明的读/写/外部副作用共用同一道宿主审批闸门。审批卡片停在输入栏内，
  工具调用则以紧凑进度行渲染（如 `Read FengYu Plugin Dev skill`）。插件清单可声明可选的
  `aiTools[].effect`，未声明的第三方副作用按保守策略对待。
- **可视化的工作流工具契约。** 插件 AI 工具可声明可选的、序列化的 `outputSchema`；官方
  Excel、Email、离线 Python 工具现已发布面向用户的输入元数据与结果信封 schema，用于画布配置。

### ♻️ 变更
- **用户在消息中点名为输出目标的目录，永不对 worker 可写。** 改为每回合创建一个插件私有的
  临时（staging）目录并作为可写的沙箱根交给 worker；回合结束后由宿主将其内容拷贝到真实目标
  目录并删除临时目录树。真实目录始终只读，因此 OS 沙箱的可写根保持稳定（每回合仅一个临时
  授权），worker 也永远无法覆盖用户命名目录中的文件。插件 worker 现在还获得一个插件私有的可写
  临时目录（`-Djava.io.tmpdir` + `TMPDIR`/`TMP`/`TEMP`）；`PLUGIN_DATA_DIR_ENV` 现在对所有插件
  都设置，而不再仅限声明了数据库权限的插件。macOS 沙箱可写根在构建 profile 前通过
  `toRealPath()` 规范化（解析 `/var` → `/private/var`）。
- **聊天工具循环上限现已可配置。** 新增 `ai.max_tool_rounds` 设置（默认 50，`0` = 无限制），
  取代此前每个后端硬编码的上限，约束单回合工具调用轮数，防止模型重复请求同一工具导致虚拟线程
  卡死、锁住后端。可在「设置 → AI」中编辑。
- **Agent 工具发现现随已安装插件的生命周期变化。** 新的运行读取实时工具注册表，而画布节点在
  工具被禁用/卸载后仍能存活、阻止不安全执行，并在同一工具再次启用时重新协调新增的必填输入。
- **升级 Apache POI 至 5.5.1。**

### 🐛 修复
- **AI 聊天现可可靠处理 Excel 文件工作流与高审批密度回复。** 附件/目录会授予给每一个兼容的
  后端插件，用户消息中显式键入的绝对路径会被转换为只读的插件作用域 FileRef。所选目录在声明的
  地方保留写访问，并注入到单一写目录插件工具参数中；拆分操作前优先做 Excel 分析，未解析的
  FileRef 对象也不再会变成 map 形态的输出目录。点击发送时 IME 组合不再丢失末尾英文段，最终
  回答也正确渲染在命令审批卡片下方。
- **生成进行中时权限模式菜单现已被禁用**，无法在回合进行中切换审批档位。
