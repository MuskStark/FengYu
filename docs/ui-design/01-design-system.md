...
> **Role:** This is the **constitution** of the SwissKitJ user interface — the design
...
> way it does, the layout primitives, the typography/spacing/radius scales, and how SwissKitJ
...
| **Window source** | [`SwissKit/src/main/java/fan/summer/app/SwissKitJApp.java`](../../SwissKit/src/main/java/fan/summer/app/SwissKitJApp.java) · [`ui/MainWindow.java`](../../SwissKit/src/main/java/fan/summer/ui/MainWindow.java) |
...
SwissKitJ is a **plugin-based desktop toolkit** built on JavaFX 21. Its user interface is a
...
Four non-negotiable principles. Every layout decision in SwissKitJ derives from one of these.

### P1 — Functional-first

SwissKitJ is a **toolbox**, not a showcase. The UI exists to get the user to a tool and let
...
A third-party plugin dropped into `.swisskit/plugin/` must be visually indistinguishable from
...
|---|---|---|
| Initial scene size | **960 × 620** | `SwissKitJApp.java:113` |
| Minimum window size | **800 × 520** | `SwissKitJApp.java:137–138` |
| Window chrome | **Native** (`StageStyle.DECORATED`) | `SwissKitJApp.java:133` |
| Title | `SwissKitJ` | `SwissKitJApp.java:134` |
...
|---|---|---|---|
| Left | [`Sidebar`](../../SwissKit/src/main/java/fan/summer/ui/sidebar/Sidebar.java) | Navigation: categories (DEV/TEXT/IMAGE/NET/OTHER), AI, Plugins, Favorites, Settings; theme toggle in footer. Collapsible (persisted as `sidebar.collapsed`). | `ui/sidebar/Sidebar.java` |
| Center | [`ContentArea`](../../SwissKit/src/main/java/fan/summer/ui/content/ContentArea.java) | Search bar + tool grid (FlowPane of `ToolCard`s) + cached plugin pages. `showPage(node, title)` swaps content with a 220/180 ms cross-fade. | `ui/content/ContentArea.java` |
| Right (overlay) | [`DetailPanel`](../../SwissKit/src/main/java/fan/summer/ui/content/DetailPanel.java) | Slide-in (300 ms) launch panel for a hovered/selected tool — icon, name, description, launch button. | `ui/content/DetailPanel.java` |
| Bottom | StatusBar (in [`MainWindow`](../../SwissKit/src/main/java/fan/summer/ui/MainWindow.java)) | Mono clock, pulsing status dot (2500 ms), status text. 28 px tall. | `ui/MainWindow.java` |
...
**Size scale.** SwissKitJ uses a tight, IDE-appropriate scale around a **13 px base**. Every
...
SwissKitJ uses a small, consistent set of corner radii (CSS `-fx-background-radius`):
...
SwissKitJ is **flat by default**. Drop shadows are a scarce resource reserved for surfaces
...
SwissKitJ adopts the IDEA New UI's *spirit* (neutral grays, restrained accent, flat surfaces,
...
| **Sidebar category sections** | Group nav by `ToolCategory` (DEV / TEXT / IMAGE / NET / OTHER) plus AI, Plugins, Favorites, Settings. Reflects SwissKit's plugin taxonomy. | `Sidebar.java` |
...
| **Notification system** | `.sk-notif-*` toasts (info/success/warning/error) for tool feedback — IDEA uses its own notification API; SwissKit exposes a themed equivalent to plugins. | see [03 · Notification](03-component-library.md) |

### Intentionally dropped from IDEA

| Dropped | Reason |
|---|---|
| Multi-split editor tabs | SwissKit shows one tool at a time (`showPage` cross-fade). Tools are not documents. |
| Toolbar / tool window buttons | The sidebar + grid is enough; an IDE-style toolbar would add chrome without value. |
| Heavy modal project structure | SwissKit is flat: launch a tool, use it, leave. No project tree. |

---

## 5. AI Development Checklist

When generating UI for SwissKitJ (host or plugin), you **MUST**:
...
**Source files:**
- [`SwissKit/src/main/java/fan/summer/app/SwissKitJApp.java`](../../SwissKit/src/main/java/fan/summer/app/SwissKitJApp.java) — window size, `StageStyle.DECORATED`, title
- [`SwissKit/src/main/java/fan/summer/ui/MainWindow.java`](../../SwissKit/src/main/java/fan/summer/ui/MainWindow.java) — layout root, StatusBar
