...
| **Source of truth** | [`SwissKitJ-Api/src/main/resources/css/swisskit-common.css`](../../SwissKitJ-Api/src/main/resources/css/swisskit-common.css) |
...
> `swisskit-common.css`, where the looked-up colors **are** in scope, so they *do* resolve
...
`swisskit-common.css`** with the exact values below. These are reproduced verbatim from the
...
(also defined in `swisskit-common.css`). This is mandatory whenever you would otherwise be
...
For richer components, `swisskit-common.css` ships ready-made classes that bundle several
...
```
  Application startup
        │
        ▼
  ThemeService.registerScene(mainScene)   ──►  loads swisskit-common.css
...
Three collaborators, all in `fan.summer.api.theme`:
...
`swisskit-common.css` once, adds the scene to the tracked list, and stamps the current theme
class on the root.
...
// Load swisskit-common.css + stamp current theme class on root.
...
public static void registerScene(Scene scene) {
    ...
    Themes.loadCommonStylesheet(scene);                 // adds /css/swisskit-common.css once
...
```java
Themes.COMMON_CSS = "/css/swisskit-common.css";   // resource inside the API JAR
...
- [ ] **Standalone `Stage`/`Scene`? Call `Themes.applyTo(scene)`.** Do not call
      ...
      `swisskit-common.css` to `getStylesheets()`.
...
```css
/* ✅ CORRECT — use an existing token, or add the token to swisskit-common.css first */
...
| What | Path |
|---|---|
| Token + utility-class definitions | [`SwissKitJ-Api/src/main/resources/css/swisskit-common.css`](../../SwissKitJ-Api/src/main/resources/css/swisskit-common.css) |
...
*Token values in this document are reproduced verbatim from `swisskit-common.css` and
