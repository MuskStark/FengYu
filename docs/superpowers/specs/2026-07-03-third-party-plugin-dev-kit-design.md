# 第三方插件开发套件(swisskitj-plugin-kit)设计
...
2. **漂移** —— 现有 skill 是 37KB 硬模板巨物(创建于 2025-06-09),内嵌
   `swisskit.api.version=3.0.0`,而主项目已到 v3.2.0(PluginHost、`.glass-*`→`.sk-*`
...
产物 `swisskitj-plugin-kit` 存放在**主仓库**(与 API 同源演进):
...
└── .claude-plugin/plugin/
    ...
    ├── skills/
    │   └── swisskitj-plugin-dev/
    │       └── SKILL.md            # 精简引导脚手架(§5)
    ├── agents/
    │   └── swisskitj-plugin-reviewer.md   # 语义合规审查 agent(§6b)
...
`/plugin marketplace add <repo>` + `/plugin install swisskitj-plugin-kit`。

## 4. 组件边界

| 组件 | 做什么 | 依赖 | 不做什么 |
|---|---|---|---|
| `swisskitj-plugin-dev` skill | 问需求 → 拼装项目 → 落地 CLAUDE.md → 触发校验 | `standards/` | 不裁定标准正确性 |
...
| `swisskitj-plugin-reviewer` agent | 语义规则审查,产出违规清单+建议 | `standards/checklist.md` | 不直接改码 |
| CLAUDE.md 模板 | 落到开发者新仓库,持续约束其 Claude 会话 | —— | —— |

## 5. 脚手架 skill 行为(`swisskitj-plugin-dev`)
...
3. 按需拼装:基础骨架必出;DB(H2+MyBatis)/ Excel(fesod-sheet)/ AI(AiTool)/
   后台任务为可选模块。生成的 `pom.xml` 中 `swisskit.api.version` 从
...
### 6b. `swisskitj-plugin-reviewer` agent —— 语义规则(脚本查不了)
