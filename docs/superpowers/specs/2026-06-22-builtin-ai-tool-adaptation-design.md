...
```java
// SwissKitJ-Api: fan.summer.fengyu.api.SwissKitJPlugin
...
```java
// SwissKit: fan.summer.fengyu.plugin.PluginRegistry
...
**关键决策:**
- **`addPlugins` 改 public**(原本 package-private)。`BuiltinToolRegistrar`(在 `fan.summer.fengyu.registrar`)需要跨包调用。
...
| 宿主插件 | 接管的 AI 工具 | 备注 |
|---|---|---|
| `Base64Plugin` | `BuiltinBase64Tool` | 工具类物理位置不变(`fan.summer.fengyu.ai.tools`) |
...
- 位置:`fan.summer.fengyu.buildintool.browser.BrowserAutomatePlugin`
...
```java
// SwissKit: fan.summer.fengyu.ai.tools.AiToolDescriptions
...
热重载:
  □ 把一个外部插件 JAR 丢进 .fengyu/plugin/ → 工具自动注册
  □ 删除 JAR → 工具自动注销
