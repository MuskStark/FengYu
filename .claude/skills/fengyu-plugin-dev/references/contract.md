# The `FengYuPlugin` Contract

The single interface every plugin implements:
`fan.summer.fengyu.api.FengYuPlugin`
([source](https://github.com/MuskStark/FengYu/blob/main/FengYu-Api/src/main/java/fan/summer/api/FengYuPlugin.java)).

The host depends only on this interface. You declare `FengYu-Api` as `provided` scope,
...
([source](https://github.com/MuskStark/FengYu/blob/main/FengYu-Api/src/main/java/fan/summer/api/host/PluginHost.java)).
...
`PluginContext`
([source](https://github.com/MuskStark/FengYu/blob/main/FengYu-Api/src/main/java/fan/summer/api/PluginContext.java))
...
The host uses `ServiceLoader.load(FengYuPlugin.class, classloader)` with a custom
...
- **Parent-first for *classes*** — `FengYuPlugin` and shared API classes resolve to the
  same `Class` object as the host (required for `instanceof`/casts/`ServiceLoader` to work).

So: bundle your own deps (H2, FesodSheet, MyBatis) at default scope; keep `FengYu-Api`
