...
**Contract**
([`AiTool` source](https://github.com/MuskStark/FengYu/blob/main/FengYu-Api/src/main/java/fan/summer/api/ai/AiTool.java)):
...
[`BuiltinJsonFormatTool.java`](https://github.com/MuskStark/FengYu/blob/main/FengYu/src/main/java/fan/summer/ai/tools/BuiltinJsonFormatTool.java).
...
- DB path convention: `.fengyu/plugins/database/pl_{{slug}}` (relative to `user.dir`).
...
[Database Layer guide](https://muskstark.github.io/FengYu/#/plugins/database).
...
`Task` (see above). Full API in the [Excel I/O guide](https://muskstark.github.io/FengYu/#/plugins/excel).
...
| Symptom | Cause | Fix |
|---|---|---|
| **Plugin doesn't appear in the host** | SPI file missing, at wrong path, or overwritten by shade | File must be `META-INF/services/fan.summer.fengyu.api.FengYuPlugin` (not `services/` at root); shade plugin needs `ServicesResourceTransformer`. Verify: `unzip -p target/*.jar META-INF/services/fan.summer.fengyu.api.FengYuPlugin` |
...
| **`*PluginUi` wrapper compiles but the host won't load it** | Wrapper isn't the SPI entry | Implement `FengYuPlugin` in ONE class; the SPI file points at it |

For the exhaustive list, see the
[Common Pitfalls doc](https://muskstark.github.io/FengYu/#/plugins/pitfalls) (12 entries —
