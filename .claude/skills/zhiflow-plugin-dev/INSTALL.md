# Installing the `zhiflow-plugin-dev` skill
...
git add .agents/skills/zhiflow-plugin-dev
...
svn export https://github.com/MuskStark/ZhiFlow/trunk/.agents/skills/zhiflow-plugin-dev \
  .agents/skills/zhiflow-plugin-dev
cp -R .agents/skills/zhiflow-plugin-dev .claude/skills/zhiflow-plugin-dev
git add .agents/skills/zhiflow-plugin-dev .claude/skills/zhiflow-plugin-dev
git commit -m "chore: add zhiflow-plugin-dev skill (ZCode + Claude Code)"
```

> No `svn`? Use `git sparse-checkout`, [`degit`](https://github.com/Rich-Harris/degit), or
> download the [tarball](https://github.com/MuskStark/ZhiFlow/archive/refs/heads/main.zip)
> and extract just `.agents/skills/zhiflow-plugin-dev/`.
...
Verify (either agent):
```bash
ls ~/.agents/skills/zhiflow-plugin-dev/SKILL.md     # ZCode
ls ~/.claude/skills/zhiflow-plugin-dev/SKILL.md     # Claude Code
```
...
> `https://github.com/MuskStark/ZhiFlow/tree/main/.agents/skills/zhiflow-plugin-dev`

This works in both ZCode (e.g. via `/zhiflow-plugin-dev`) and Claude Code (e.g. `@skill` or
...
that fire it: *"scaffold a ZhiFlow plugin"*, *"fix my plugin that won't load"*, *"add an AI
tool to my plugin"*, *"implement ZhiFlowPlugin"*, mentions of `.zhiflow/plugin` or the
plugin SPI.

- **ZCode** — auto-discovers; force-load with `/zhiflow-plugin-dev <request>`.
...
The `name: zhiflow-plugin-dev` and directory name stay stable across updates, so there's no
migration.
