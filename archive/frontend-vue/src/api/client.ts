...
export const api = {
  ...
  /** Read-only workspace browsing for the chat view's side panel (tree + single file). */
  async getWorkspaceTree(id: number): Promise<WorkspaceTree> {
    const { data } = await http.get<WorkspaceTree>(
      `/api/ai/conversations/${id}/workspace/tree`)
    return data
  },

  async getWorkspaceFile(id: number, path: string): Promise<WorkspaceFilePreview> {
    const { data } = await http.get<WorkspaceFilePreview>(
      `/api/ai/conversations/${id}/workspace/file`, { params: { path } })
    return data
  },
