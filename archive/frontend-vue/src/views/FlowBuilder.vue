...
import { FLOW_CHAT_SEED_KEY, type FlowChatSeed } from '@/components/chat/flowSeed'
import {
  MarkerType,
  VueFlow,
  useVueFlow,
...
/** Pending "@ → flow" prefill for the docked chat; cleared once the panel consumes it. */
const chatSeed = ref<string | null>(null)
...
/** Consume the chat composer's "@ → flow" hand-off: prefill + open the docked chat. */
function consumeChatSeed() {
  try {
    ...
    if (seed.workflowId === workflowId.value && seed.text?.trim()) {
      chatSeed.value = seed.text
      chatOpen.value = true
    }
  } catch {
        ...
        <transition name="flow-chat-slide">
          <aside v-if="chatOpen" class="flow-chat-dock">
            <FlowChatPanel
              ...
              :seed-prompt="chatSeed ?? undefined"
              @seed-consumed="chatSeed = null"
