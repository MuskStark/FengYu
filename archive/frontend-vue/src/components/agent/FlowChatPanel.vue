...
const props = defineProps<{
  ...
  /**
   * One-shot prefill from the chat composer's "@ → flow" hand-off: the typed input lands in
   * this dock's box, ready to send. Consumed once via the `seed-consumed` event.
   */
  seedPrompt?: string
}>()
const emit = defineEmits<{ close: []; 'seed-consumed': [] }>()
