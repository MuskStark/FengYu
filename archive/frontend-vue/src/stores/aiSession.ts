...
import type { MentionOption } from '@/components/chat/mentionSearch'
...
export interface Conversation {
  ...
  turns: ChatTurn[]
  createdAt: number
  /** Last message time (creation time until a turn completes) — the sidebar's activity sort. */
  updatedAt?: number
  ...
  draft: string
  /**
   * Inline mention tokens currently in the composer for this conversation (session memory only,
   * like draft attachments): restored on switch, serialized as markdown on send.
   */
...
export const useAiSessionStore = defineStore('aiSession', () => {
    ...
    try {
      ...
      const fresh = list
        ...
        .map((s) => ({
          ...
          updatedAt: Date.parse(s.updatedAt) || Date.parse(s.createdAt) || Date.now(),
          loaded: false,
          draft: '',
          draftMentions: [],
          draftAttachments: [],
  ...
  async function loadConversationTurns(conv: Conversation): Promise<boolean> {
    try {
      ...
      error.value = i18n.global.t('aichat.loadConversationFailed')
      return false
    }
  }
  ...
  async function doPersist(conv: Conversation) {
    ...
    try {
      ...
      error.value = i18n.global.t('aichat.conversationSaveFailed')
    }
  }
  ...
  async function send(text: string) {
    ...
    try {
      ...
      error.value = e instanceof Error ? e.message : i18n.global.t('aichat.startFailed')
      restoreDraft(conv, prompt)
      abandon()
      return
    }
    ...
    try {
      ...
      error.value = e instanceof Error ? e.message : i18n.global.t('aichat.prepareFailed')
