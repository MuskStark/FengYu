...
const deleteContact = (id: number) => { if (window.confirm(t('contacts.deleteConfirm'))) void run(t('contacts.deleteAction'), () => invoke('email_contact_delete', { id })) }
const addTag = () => run(t('contacts.tagSaveAction'), async () => { await invoke('email_tag_save', { name: tagName.value }); tagName.value = '' })
const deleteTag = (id: number) => { if (window.confirm(t('contacts.tagDeleteConfirm'))) void run(t('contacts.tagDeleteAction'), () => invoke('email_tag_delete', { id })) }
