...
<template><section class="panel-grid"><v-card class="surface" variant="flat"><v-card-title>Contacts</v-card-title><v-card-text><v-alert v-if="error" type="error" class="mb-4">{{error}}</v-alert><div class="inline-fields"><v-text-field v-model="store.query" label="Search" @keyup.enter="store.load"/><v-btn @click="store.load">Search</v-btn></div><v-list lines="two"><v-list-item v-for="item in store.contacts" :key="item.id" :title="item.nickname||item.email" :subtitle="item.email" @click="edit(item)"><template #prepend><v-checkbox-btn v-model="selectedContacts" :value="item.id" @click.stop/></template><template #append><v-btn variant="text" color="error" @click.stop="deleteContact(item.id)">Delete</v-btn></template></v-list-item></v-list><div class="inline-fields mt-4"><v-select v-model="assignTagIds" :items="store.tags" item-title="name" item-value="id" multiple chips label="Assign tags to selected"/><v-btn :disabled="!selectedContacts.length" @click="assign">Assign</v-btn></div></v-card-text></v-card>
