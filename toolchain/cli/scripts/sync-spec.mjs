#!/usr/bin/env node
// Copies the canonical ../spec/manifest.schema.json into cli/spec/
...
// is always toolchain/spec/manifest.schema.json; this generated copy is never edited.
import fs from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'

const here = path.dirname(fileURLToPath(import.meta.url))
const source = path.resolve(here, '../../spec/manifest.schema.json')
...
if (checking) {
  ...
  if (!current || !current.equals(buffer)) {
    console.error(`spec/manifest.schema.json is out of sync with toolchain/spec/manifest.schema.json. Run: npm run sync-spec`)
    process.exit(1)
  }
