# Installing the `fengyu-plugin-dev` skill
...
[`MuskStark/FengYu/.agents/skills/fengyu-plugin-dev/`](https://github.com/MuskStark/FengYu/tree/main/.agents/skills/fengyu-plugin-dev).
...
svn export https://github.com/MuskStark/FengYu/trunk/.agents/skills/fengyu-plugin-dev \
  .agents/skills/fengyu-plugin-dev
git add .agents/skills/fengyu-plugin-dev
git commit -m "chore: add fengyu-plugin-dev skill"
```
...
svn export https://github.com/MuskStark/FengYu/trunk/.agents/skills/fengyu-plugin-dev \
  .claude/skills/fengyu-plugin-dev
git add .claude/skills/fengyu-plugin-dev
git commit -m "chore: add fengyu-plugin-dev skill"
```
...
svn export https://github.com/MuskStark/FengYu/trunk/.agents/skills/fengyu-plugin-dev \
  .agents/skills/fengyu-plugin-dev
cp -R .agents/skills/fengyu-plugin-dev .claude/skills/fengyu-plugin-dev
git add .agents/skills/fengyu-plugin-dev .claude/skills/fengyu-plugin-dev
git commit -m "chore: add fengyu-plugin-dev skill (ZCode + Claude Code)"
```

> No `svn`? Use `git sparse-checkout`, [`degit`](https://github.com/Rich-Harris/degit), or
> download the [tarball](https://github.com/MuskStark/FengYu/archive/refs/heads/main.zip)
> and extract just `.agents/skills/fengyu-plugin-dev/`.
...
Verify (either agent):
```bash
ls ~/.agents/skills/fengyu-plugin-dev/SKILL.md     # ZCode
ls ~/.claude/skills/fengyu-plugin-dev/SKILL.md     # Claude Code
```
...
> `https://github.com/MuskStark/FengYu/tree/main/.agents/skills/fengyu-plugin-dev`

This works in both ZCode (e.g. via `/fengyu-plugin-dev`) and Claude Code (e.g. `@skill` or
...
that fire it: *"scaffold a FengYu plugin"*, *"fix my plugin that won't load"*, *"add an AI
tool to my plugin"*, *"implement FengYuPlugin"*, mentions of `.fengyu/plugin` or the
plugin SPI.

- **ZCode** — auto-discovers; force-load with `/fengyu-plugin-dev <request>`.
...
The `name: fengyu-plugin-dev` and directory name stay stable across updates, so there's no
migration.
