...
description: Build, scaffold, debug, and theme FengYu plugins — external JAR tools that implement the `FengYuPlugin` interface and load into the FengYu JavaFX host. Use whenever the user wants to create a FengYu plugin/tool, work with the plugin API (FengYuPlugin, ToolCategory, IconStyle, MdiIconUtil, I18n, Themes), add an AI tool to a plugin, fix a plugin that won't load or renders unthemed, scaffold a plugin project, or mentions FengYu plugin development, `.fengyu/plugin`, plugin SPI / ServiceLoader, or the FengYu-Api. Also use when editing code that `implements FengYuPlugin` or that lives under `FengYu-Api`.
---

# FengYu Plugin Development

You are an expert author of **FengYu plugins** — external JAR tools that implement
`fan.summer.fengyu.api.FengYuPlugin`, register via Java `ServiceLoader` SPI, and hot-load into the
FengYu JavaFX host (dropped into `.fengyu/plugin/`). This skill makes you produce plugins
...
The FengYu host is a JavaFX 21 app implementing the JetBrains IDEA 2025 "New UI". Plugins
...
> inside the FengYu repo, the official plugin repo, and a third-party developer's project:
...
- **Docs site (rendered, best for reading):** `https://muskstark.github.io/FengYu/`
- **Source (best for citing `file:line`):** `https://github.com/MuskStark/FengYu/blob/main/`
- **UI design spec (the authority):** `docs/ui-design/` → e.g.
  `https://muskstark.github.io/FengYu/#/ui-design/05-theme-color-system` (docsify) or
  `https://github.com/MuskStark/FengYu/blob/main/docs/ui-design/05-theme-color-system.md` (raw)
- **API source root:** `FengYu-Api/src/main/java/fan/summer/api/`
...
> [plugins docs](https://muskstark.github.io/FengYu/#/plugins/) as supporting context, not
> authoritative.
...
palette). The JavaFX `FengYuPlugin` + `createView()` contract described in the rest of this skill
is the historical v3.x API, retained for the still-JavaFX `FengYu-Api` preview classes.
...
[`buildintool/`](https://github.com/MuskStark/FengYu/tree/main/FengYu/src/main/java/fan/summer/buildintool),
...
| **Edit/extend an existing plugin** | Find the `*Plugin.java` (implements `FengYuPlugin`); re-read [references/contract.md](references/contract.md) for the method you're touching |
...
1. **One class, implements `FengYuPlugin` directly.** No separate `*PluginUi` /
...
6. **SPI registration file is mandatory and path-sensitive.**
   - Path: `src/main/resources/META-INF/services/fan.summer.fengyu.api.FengYuPlugin`
   ...
   - The `maven-shade-plugin` MUST include `ServicesResourceTransformer` or the file gets
     ...
     `unzip -p target/*.jar META-INF/services/fan.summer.fengyu.api.FengYuPlugin`
...
8. **UI MUST follow the main project's UI design spec.** The plugin is a guest in the
   FengYu shell — it must blend in as native, never invent its own look. The authoritative
   design system is the **`docs/ui-design/`** doc set (rendered:
   `https://muskstark.github.io/FengYu/#/ui-design/`; source:
   `https://github.com/MuskStark/FengYu/blob/main/docs/ui-design/`).
   ...
   - **Colors** — `-sk-*` tokens / `.sk-*` classes only, **never inline hex** (breaks on theme
     ...
     [05 Theme & Color System](https://muskstark.github.io/FengYu/#/ui-design/05-theme-color-system).
   - **Components** — use the exact `.sk-*` classes specified in
     [03 Component Library](https://muskstark.github.io/FengYu/#/ui-design/03-component-library):
   ...
   - **Typography** — global font stack `"SF Pro Text", "Inter", "Segoe UI", "PingFang SC",
     ...
     ([01 Design System](https://muskstark.github.io/FengYu/#/ui-design/01-design-system)).
   - **Motion** — reuse the token durations/easings; interaction feedback ≤ 300 ms; **never
     animate the theme switch**
     ([07 Animation Guidelines](https://muskstark.github.io/FengYu/#/ui-design/07-animation-guidelines)).
   - **Interaction & a11y** — cache the view; cross-fade on page switch; confirm destructive
     ops; show one of loading/empty/error/success
     ([04](https://muskstark.github.io/FengYu/#/ui-design/04-interaction-guidelines));
     ≥ 4.5:1 contrast, never status-by-color-alone, keyboard-reachable, Esc closes
     ([08 Accessibility](https://muskstark.github.io/FengYu/#/ui-design/08-accessibility-guide)).
...
public class CsvSorterPlugin implements FengYuPlugin {
