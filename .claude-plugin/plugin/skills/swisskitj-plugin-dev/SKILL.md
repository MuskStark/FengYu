---
name: fengyuj-plugin-dev
description: Use when a developer wants to create a new FengYu plugin. Scaffolds a standards-compliant plugin project in an independent repo and runs compliance checks.
---

# FengYu Plugin Scaffolder

Scaffolds a new FengYu plugin into the developer's **own, independent repo** (not this
...
2. **Plugin ID** (reverse-domain, e.g. `plugin.fengyu.star`) — at least two dot-separated segments; this becomes `getId()`.
3. **Base package** (e.g. `plugin.fengyu.star`) — Java package root. Often identical to the plugin ID.
...
- `../../standards/VERSION` — the current FengYu API version. Use this exact string as
  the `fengyu.api.version` property value in the generated `pom.xml`. **Never hardcode a
...
│   ├── <Name>Plugin.java          # SPI entry point (implements FengYuPlugin)
...
└── src/main/resources/
    ├── META-INF/services/fan.summer.fengyu.api.FengYuPlugin
    └── i18n/
...
- `FengYu-Api` dependency, `scope=provided`, `<version>${fengyu.api.version}</version>`
  where `fengyu.api.version` is a `<properties>` entry set to the value read in Step 2.
...
**`<Name>Plugin.java`** — implements `fan.summer.fengyu.api.FengYuPlugin`. Follow
...
**SPI file** — `src/main/resources/META-INF/services/fan.summer.fengyu.api.FengYuPlugin`,
...
1. Read `../../templates/CLAUDE.md.tmpl`, fill its `{{...}}` placeholders with the values
   collected in Step 1 (and the `fengyu.api.version` read in Step 2), and write the result
...
Then invoke the `fengyuj-plugin-reviewer` agent against the new project directory to check
...
`fengyuj-plugin-reviewer` agent reports `SEMANTIC OK`.
