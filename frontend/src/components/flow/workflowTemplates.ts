...
/**
 * Built-in one-click workflow templates (port of the Vue workflowTemplates.ts): a
 * pre-wired node graph plus a run-form input schema an ordinary (non-technical) user
 * fills in — no tool knowledge required. Titles, descriptions and goals are i18n keys
 * resolved when the template is applied.
 */

/** Catalog option source: a plugin list method the host fetches options from. */
export interface WorkflowTemplateEnumSource {
  plugin: string
  method: string
  items?: string
  value?: string
...
export interface WorkflowTemplateProperty {
  ...
  analyze?: 'excel'
  enumSource?: WorkflowTemplateEnumSource
...
export function templateMissingTools(template: WorkflowTemplate, tools: AgentTool[]): string[] {
  return template.requiredTools.filter((name) => !tools.some((tool) => tool.name === name))
}
