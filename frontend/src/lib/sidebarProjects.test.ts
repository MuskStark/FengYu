...
import { formatRelativeTime, groupConversations, sortForView } from './sidebarProjects'
import type { SidebarConversation } from './sidebarProjects'

function conv(id: number, workspaceRoot: string | null, createdAt = id, updatedAt?: number): SidebarConversation {
  return { id, title: `c${id}`, createdAt, workspaceRoot, updatedAt }
}
...
describe('formatRelativeTime (ZCode taskList thresholds)', () => {
  ...
  it('buckets elapsed time into justNow / minutes / hours / days', () => {
    expect(formatRelativeTime(now - 30_000, now)).toEqual({ key: 'sidebar.justNow' })
    expect(formatRelativeTime(now - 5 * MINUTE, now)).toEqual({ key: 'sidebar.minutesAgo', values: { minutes: 5 } })
    expect(formatRelativeTime(now - 3 * HOUR, now)).toEqual({ key: 'sidebar.hoursAgo', values: { hours: 3 } })
    expect(formatRelativeTime(now - 12 * 24 * HOUR, now)).toEqual({ key: 'sidebar.daysAgo', values: { days: 12 } })
  })

  it('never produces negative buckets for future timestamps', () => {
    expect(formatRelativeTime(now + 10 * MINUTE, now)).toEqual({ key: 'sidebar.justNow' })
  })

  it('crosses each threshold boundary exactly', () => {
    expect(formatRelativeTime(now - MINUTE, now)).toEqual({ key: 'sidebar.minutesAgo', values: { minutes: 1 } })
    expect(formatRelativeTime(now - 60 * MINUTE, now)).toEqual({ key: 'sidebar.hoursAgo', values: { hours: 1 } })
    expect(formatRelativeTime(now - 24 * HOUR, now)).toEqual({ key: 'sidebar.daysAgo', values: { days: 1 } })
  })
})
